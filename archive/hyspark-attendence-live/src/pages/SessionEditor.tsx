...
import { PageHeader, PageShell, Surface } from '@/components/app-ui';
import { mergeSessionNoteText, parseSessionNotes } from '@/lib/networking';
...
export default function SessionEditor() {
  ...
  useEffect(() => {
    if (editing) {
      ...
      setForm({
        ...
        notes: parseSessionNotes(editing.notes).text,
      });
    }
  }, [editing]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isEdit) {
      await updateSession({
        ...
        notes: mergeSessionNoteText(editing!.notes, form.notes),
      });
    } else {
  ...
  return (
    <PageShell size="sm" className="space-y-6">
      ...
      />
      <Surface className="animate-reveal-up overflow-hidden" style={{ animationDelay: '60ms' }}>
        <div className="border-b border-border/60 px-5 py-4">
          <div className="flex items-center gap-2">
            <CalendarPlus className="h-4 w-4 text-muted-foreground" />
            <h2 className="text-sm font-extrabold">세션 정보</h2>
          </div>
        </div>
        <div className="p-5">

...
type Row = AttendanceRecord | PendingRow;
        ...
        <CardContent className="p-0">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {allRows.map(r => {
                  const isPending = '_pending' in r && r._pending;
                  return (
                    <tr key={r.id} className="border-b last:border-0 hover:bg-secondary/30 transition-colors">
                      <td className="py-3 px-4">
                        <div className="font-medium">{r.member_name}</div>
                        {!isPending && (r.status === 'excused_absent' || r.status === 'unexcused_absent') && ('exception_category' in r) && r.exception_category && (

...
DROP FUNCTION IF EXISTS public.submit_camp_daily_response(uuid, text, text);
...
GRANT EXECUTE ON FUNCTION public.submit_camp_daily_response(uuid, text, text, boolean) TO anon, authenticated;
...
SET search_path = public
AS $$
  SELECT r.id, r.response_date, r.attended, r.from_time, r.to_time, r.duration_minutes, r.demerit_credit

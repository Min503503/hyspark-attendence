-- 캠프 설문: 월~금 날짜 선택 제출 (기존 "오늘"만 저장 → 선택한 평일)

CREATE OR REPLACE FUNCTION public.hyspark_camp_response_json(p_resp public.camp_daily_responses)
RETURNS jsonb
LANGUAGE sql
IMMUTABLE
...
AS $$
  SELECT jsonb_build_object(
    'id', p_resp.id,
    'attended', p_resp.attended,
    'from_time', to_char(p_resp.from_time, 'HH24:MI'),
    'to_time', to_char(p_resp.to_time, 'HH24:MI'),
    'time_slots', COALESCE(to_jsonb(p_resp.time_slots), '[]'::jsonb),
    'duration_minutes', p_resp.duration_minutes,
    'demerit_credit', p_resp.demerit_credit,
    'submitted_at', p_resp.submitted_at
  );
$$;
...
DECLARE
  ...
  v_days jsonb := '[]'::jsonb;
  v_d date;
  v_weekday_labels text[] := ARRAY['일', '월', '화', '수', '목', '금', '토'];
BEGIN
  ...
  v_d := v_camp.start_date;
  WHILE v_d <= v_camp.end_date LOOP
    IF extract(dow FROM v_d) BETWEEN 1 AND 5 THEN
      SELECT * INTO v_resp
      FROM public.camp_daily_responses
      WHERE profile_id = p_profile_id AND response_date = v_d;

      v_days := v_days || jsonb_build_array(jsonb_build_object(
        'date', v_d,
        'weekday_label', v_weekday_labels[extract(dow FROM v_d)::integer + 1],
        'response', CASE
          WHEN v_resp.id IS NULL THEN NULL
          ELSE public.hyspark_camp_response_json(v_resp)
        END
      ));
    END IF;
    v_d := v_d + 1;
...
DROP FUNCTION IF EXISTS public.submit_camp_daily_response(uuid, text, text, boolean, text[]);
...
DECLARE
  ...
  v_today date := public.hyspark_kst_date();
  v_target date := COALESCE(p_response_date, v_today);
  v_from time;
  v_to time;
  v_duration integer;
...
BEGIN
  ...
  IF extract(dow FROM v_target) NOT BETWEEN 1 AND 5 THEN
    RETURN jsonb_build_object('success', false, 'message', '평일(월~금)만 선택할 수 있습니다.');
  END IF;

  IF v_target < v_camp.start_date OR v_target > v_camp.end_date THEN
    RETURN jsonb_build_object('success', false, 'message', '캠프 기간이 아닙니다.');
  END IF;

  IF v_target > v_today THEN
    RETURN jsonb_build_object('success', false, 'message', '미래 날짜는 제출할 수 없습니다.');
  END IF;
  ...
  INSERT INTO public.camp_daily_responses (
    ...
    v_camp.id, p_profile_id, v_target, v_from, v_to, v_sorted,
  ...
  v_label := v_weekday_labels[extract(dow FROM v_target)::integer + 1];

  RETURN jsonb_build_object(
    'success', true,
    'message', CASE
      WHEN v_row.attended THEN format('%s요일 참여 시간이 저장되었습니다.', v_label)
      ELSE format('%s요일 출석 안 함으로 기록되었습니다.', v_label)
    END,
    'response', public.hyspark_camp_response_json(v_row)
  );
END;
$$;

GRANT EXECUTE ON FUNCTION public.submit_camp_daily_response(uuid, text, text, boolean, text[], date) TO anon, authenticated;
